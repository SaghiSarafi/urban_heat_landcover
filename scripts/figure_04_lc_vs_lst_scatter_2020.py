...
for idx, (row, col) in enumerate(positions):
    ...
    ax.scatter(x, y_mean, c=mean_color, marker='o', s=70, alpha=0.7,
               edgecolors='white', linewidth=0.5, zorder=3, label='Mean LST')
    ax.scatter(x, y_max, c=max_color, marker='s', s=80, alpha=0.7,
               edgecolors='white', linewidth=0.5, zorder=3, label='Max LST')
    ax.scatter(x, y_top10, c=top10_color, marker='^', s=65, alpha=0.7,
               edgecolors='white', linewidth=0.5, zorder=3, label='Mean Top-10% LST')
    ...
    if p_mean < alpha_sig:
        ...
        p_fit = np.poly1d(z)
        ax.plot(np.sort(x), p_fit(np.sort(x)), color=fit_mean_color, linewidth=2.5, linestyle='-')

    r_max, p_max, _ = correlations[lc_name]['max']
    if p_max < alpha_sig:
        z = np.polyfit(x, y_max, 1)
        p_fit = np.poly1d(z)
        ax.plot(np.sort(x), p_fit(np.sort(x)), color=fit_max_color, linewidth=2.5, linestyle='-')

    r_top10, p_top10, _ = correlations[lc_name]['top10']
    if p_top10 < alpha_sig:
        z = np.polyfit(x, y_top10, 1)
        p_fit = np.poly1d(z)
        ax.plot(np.sort(x), p_fit(np.sort(x)), color=fit_top10_color, linewidth=2.5, linestyle='-')

    ax.set_title(lc_name, fontsize=16, fontweight='bold', pad=10)

    # Add stats text box
    r_mean, p_mean, n = correlations[lc_name]['mean']
    sig = '*' if p_mean < alpha_sig else 'ns'
...
fig.text(0.5, 0.02, 'Percent Land Cover (%)', ha='center', fontsize=18, fontweight='bold')
fig.text(0.01, 0.5, 'LST (K)', va='center', rotation='vertical', fontsize=18, fontweight='bold')
...
legend_elements = [
    Line2D([0], [0], marker='o', color='w', markerfacecolor=mean_color, markersize=12, label='Mean LST'),
    Line2D([0], [0], marker='s', color='w', markerfacecolor=max_color, markersize=12, label='Max LST'),
    Line2D([0], [0], marker='^', color='w', markerfacecolor=top10_color, markersize=15, label='Mean Top-10% LST'),
    Line2D([0], [0], color=fit_mean_color, linewidth=2.5, label='Mean LST fit (p<0.05)'),
    Line2D([0], [0], color=fit_max_color, linewidth=2.5, label='Max LST fit (p<0.05)'),
    Line2D([0], [0], color=fit_top10_color, linewidth=2.5, label='Top-10% LST fit (p<0.05)'),
]
ax_legend.legend(handles=legend_elements, loc='center', fontsize=14, frameon=True)

plt.tight_layout(rect=[0.02, 0.04, 1, 0.98])
plt.savefig("../outputs/figures/figure_04_lc_vs_lst_scatter_2020.png", dpi=300, bbox_inches='tight')
plt.show()