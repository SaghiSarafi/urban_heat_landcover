...
A boundary-sensitivity robustness check (Level 1, Table 5b) also tests whether administrative boundary characteristics — how much of a city's real urban extent the boundary captures, and how compact or irregular its shape is — confound the net-change results. Neither covariate is a significant predictor, and core findings are unaffected.
...
│ ├── table_05_net_change_regression.py ← Table 5a/5b (net change regression, original + boundary-augmented)
...
Generated by `GEE/03_boundary_covariates_ghsl_compactness.js`. Used in Table 5b. See `data/README_data.md` for full details.
...
- **Purpose:** For each city, compute an area ratio (boundary area relative to its GHS-SMOD Urban Centre extent) and a perimeter-area ratio (shape compactness), used in Table 5b to test whether boundary characteristics confound the Level 1 net-change results.
...
table_05_net_change_regression.py → Table 5a and Table 5b (requires boundary_covariates_ghsl_compactness.csv)
...
Plots land cover percentage composition per year across all 100 cities — bold lines for cross-city mean, thin lines for individual city trajectories. LC-class colors match Figure 2's palette.
...
**Key result:** Impervious cover change survives correction across all three metrics (corrected p = 0.037); cropland change survives for mean LST only (corrected p = 0.012). These bivariate correlations motivate, but are not the primary evidence for, the multivariate regression results in Table 5a.

---

**`table_05_net_change_regression.py`**
OLS multiple regression of city-level LST change (ΔLST, ΔLSTMax, ΔLSTMax10) on net percentage change in each LC class between 2000 and 2020. Implements **Level 1** of the hierarchical framework. Outputs two models: Table 5a (original 5-predictor model) and Table 5b (augmented with two boundary-sensitivity covariates).

**Inputs:** `data/city_year_lst_lc.csv`, `data/boundary_covariates_ghsl_compactness.csv`
**Outputs:** `outputs/tables/table_05a_net_change_regression.csv`, `outputs/tables/table_05b_net_change_regression_augmented.csv`
**Model:** `statsmodels` OLS with intercept; 5 LC predictors (5a) or 5 LC predictors + 2 boundary covariates (5b) entered simultaneously
**Key result:** Table 5a: R² ≈ 0.186; impervious (β = +0.1696 K/%, p = 0.016) and cropland loss (β = −0.2173 K/%, p = 0.031) are significant predictors. Table 5b: neither boundary covariate is significant (all p > 0.14); core findings unaffected. Baku City is excluded from both models — see `data/README_data.md`.
